...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for action in actions:
            val.append(self.getQValue(state,action))
        return max(val)
        #util.raiseNotDefined()
    ...
    def computeActionFromQValues(self, state):
        ...
        nextActions=[action for action in actions if self.getQValue(state,action)==maxVal]
        return random.choice(nextActions)
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        nowVal=self.qVal[(state,action)]
        nextVal=reward+self.discount*self.computeValueFromQValues(nextState)
        self.qVal[(state, action)]=(1-self.alpha)*nowVal+self.alpha*nextVal
        #util.raiseNotDefined()
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature in features:
            appQval+=self.weights[feature]*features[feature]
        return appQval
